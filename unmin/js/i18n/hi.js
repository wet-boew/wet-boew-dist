/*! Web Experience Toolkit (WET) / Boîte à outils de l'expérience Web (BOEW) wet-boew.github.io/wet-boew/License-en.html / wet-boew.github.io/wet-boew/Licence-fr.html
 - v4.0.0-a1-development - 2013-11-07
*/
/*
 * Web Experience Toolkit (WET) / Boîte à outils de l"expérience Web (BOEW)
 * wet-boew.github.io/wet-boew/License-en.html / wet-boew.github.io/wet-boew/Licence-fr.html
 */
/*
----- Hindi dictionary (il8n) ---
 */
( function( window ) {
"use strict";
/* main index */
var ind = {
	"%lang-code": "hi",
	"%lang-en": "Hindi",
	"%lang-fr": "hindi",
	"%lang-nat": "मानक हिन्दी",
	"%all": "सब",
	"%home": "घर",
	"%main-page": "मुख्य पृष्ठ",
	"%tphp": "पृष्ठ का शीर्ष",
	"%you-are-in": "तुम में हैं:",
	"%welcome-to": "में आपका स्वागत है:",
	"%load": "लोड हो रहा है ...",
	"%process": "प्रसंस्करण ...",
	"%srch": "खोजें",
	"%srch-terms": "शब्द (ओं) के लिए खोज:",
	"%no-match": "कोई मैच मिल गया",
	"%matches": {
		"mixin": "[MIXIN] मैचों मिला"
	},
	"%menu": "मेनू",
	"%settings": "सेटिंग्स",
	"%langs": "भाषाएँ",
	"%about": "के बारे में",
	"%curr": "(वर्तमान)",
	"%hide": "छिपाना",
	"%err": "त्रुटि",
	"%colon": ":",
	"%hyphen": " - ",
	"%full-stop": ".",
	"%comma-space": ", ",
	"%space": "&#32;",
	"%start": "प्रारंभ",
	"%stop": "रोक",
	"%back": "वापस",
	"%cancel": "रद्द करें",
	"%new-win": " (नई विंडो में खुलता है)",
	"%min-ago": "एक मिनट पहले",
	"%coup-mins": "पहले की जोड़ी",
	"%mins-ago": {
		"mixin": "[MIXIN] मिनट पहले"
	},
	"%hour-ago": "एक घंटे पहले",
	"%hours-ago": {
		"mixin": "[MIXIN] घंटे पहले"
	},
	"%days-ago": {
		"mixin": "[MIXIN] दिन पहले"
	},
	"%yesterday": "कल",

	"%nxt": "अगला",
	"%nxt-r": "अगला (सही तीर कुंजी)",
	"%prv": "पिछला",
	"%prv-l": "पिछला (बाएँ तीर कुंजी)",
	"%first": "पहले",
	"%last": "अंतिम",
	"%close-esc": "बंद कर(भागने कुंजी)",
	"%show": "दिखाना",

	/* Archived Web page template */
	"%arch-pg": "इस वेब पेज वेब पर संग्रहीत किया गया है.",
	/* Menu bar */
	"%sm-hlp": "(कुंजी और भागने की कुंजी के साथ बंद दर्ज सबमेनू खुला)",
	/* Tabbed interface */
	"%tab-rot": {
		"off": "टैब रोटेशन रोकें",
		"on": "टैब रोटेशन प्रारंभ"
	},
	"%tab-list": "टैब सूची",
	"%tab-pnl-end1": "इस टैब पैनल के अंत.",
	"%tab-pnl-end2": "टैब सूची पर लौटें",
	"%tab-pnl-end3": "या पृष्ठ के आराम करने के लिए जारी है.",
	/* Multimedia player */
	"%play": "खेल",
	"%pause": "ठहराव",
	"%open": "खुला",
	"%close": "पास",
	"%rew": "उल्टा",
	"%ffwd": "तेजी से आगे",
	"%mute": {
		"on": "म्यूट",
		"off": "अनम्यूट"
	},
	"%cc": {
		"off": "बंद कैप्शन छुपाएं",
		"on": "बंद कैप्शन दिखाएं"
	},
	"%cc-err": "लोड करने में त्रुटि बंद कैप्शन",
	"%adesc": {
		"on": "ऑडियो विवरण सक्षम",
		"off": "ऑडियो विवरण अक्षम"
	},
	"%prog-bar": "बायाँ तीर और दाएँ तीर कुंजी का उपयोग करने के लिए अग्रिम और मीडिया प्रगति अतीत",
	"%no-video": "आपका ब्राउज़र क्षमताओं इस वीडियो खेलने के लिए प्रकट नहीं होता है, नीचे दिए गए वीडियो डाउनलोड करें",
	"%pos": "वर्तमान स्थिति:",
	"%perc": "प्लेबैक प्रतिशत:",
	"%dur": "कुल समय:",
	"%buff": "बफ़र्ड:",
	/* Share widget */
	"%fav": "पसंदीदा",
	"%email": "ईमेल",
	"%shr-txt": "शेयर इस पन्ने",
	"%shr-hnt": "{s} के साथ",
	"%shr-email-sub": "दिलचस्प पेज",
	"%shr-email-bd": "मैंने सोचा था कि आप इस पेज दिलचस्प मिल सकता है:\n{t} ({u})",
	"%shr-fav-ttl": "(इस पृष्ठ को बुकमार्क)",
	"%shr-man": "इस संवाद को बंद करें और प्रेस Ctrl-D इस पृष्ठ को बुकमार्क करें.",
	"%shr-all": "सभी दिखाएं ({n})",
	"%shr-all-ttl": "सभी साइटों को बुकमार्क",
	"%shr-disc": "किसी भी उत्पाद या सेवाओं की कोई समर्थन व्यक्त या निहित",
	/* Form validation */
	"%frm-nosubmit": "फार्म प्रस्तुत नहीं किया जा सका क्योंकि ",
	"%errs-fnd": " त्रुटियों पाया गया.",
	"%err-fnd": " त्रुटि पाया गया था.",
	/* Date picker */
	"%date-hide": "कैलेंडर छिपाएँ",
	"%date-show": "कैलेंडर से क्षेत्र के लिए एक तारीख उठाओ",
	"%date-sel": "चयनित",
	/* Calendar */
	"%days": ["रविवार", "सोमवार", "मंगलवार", "बुधवार", "बृहस्पतिवार", "शुक्रवार", "शनिवार"],
	"%mnths": ["जनवरी", "फरवरी", "मार्च", "अप्रैल", "मई", "जून", "जुलाई", "अगस्त", "सितंबर", "अक्टूबर", "नवंबर", "दिसंबर"],
	"%cal": "कैलेंडर",
	"%currDay": "(वर्तमान दिन)",
	"%cal-goToLnk": "<span class=\"wb-inv\">वर्ष के महीने के लिए </span>जाओ",
	"%cal-goToTtl": "वर्ष के महीने के लिए जाओ",
	"%cal-goToMnth": "माह:",
	"%cal-goToYr": "वर्ष:",
	"%cal-goToBtn": "जाना",
	"%prvMnth": "पिछले माह: ",
	"%nxtMnth": "अगला महीना: ",
	/* Slideout */
	"%show-toc": "सामग्री की तालिका",
	"%hide-toc": "सामग्री की तालिका छुपाएं",
	"%toc": "सामग्री तालिका",
	/* Lightbox */
	"%lb-curr": "%total% में से %curr% आइटम",
	"%lb-xhr-err": "इस सामग्री को लोड करने में विफल.",
	"%lb-img-err": "इस छवि को लोड करने में विफल रहा.",
	/* Charts widget */
	"%table-mention": "तालिका",
	"%table-following": "चार्ट. निम्न तालिका में विवरण.",
	/* Session timeout */
	"%st-to-msg-bgn": "आपका सत्र #min# मिनट #sec# सेकंड में स्वत: ही समाप्त हो जाएगी.",
	"%st-to-msg-end": "अपने सत्र का विस्तार करने के लिए \"सत्र जारी\" का चयन करें.",
	"%st-msgbx-ttl": "सत्र मध्यांतर चेतावनी",
	"%st-alrdy-to-msg": "खेद अपने सत्र पहले ही समाप्त हो गया है. कृपया फिर से प्रवेश करें.",
	"%st-btn-cont": "सत्र जारी",
	"%st-btn-end": "अब सत्र समाप्त",
	/* Toggle details */
	"%td-toggle": "सभी टॉगल करें",
	"%td-open": "सभी का विस्तार",
	"%td-close": "सभी को संक्षिप्त करें",
	"%td-ttl-open": "सामग्री के सभी वर्गों का विस्तार",
	"%td-ttl-close": "सामग्री के सभी वर्गों को संक्षिप्त करें",
	/* Table enhancement */
	"%sortAsc": ": तरह आरोही के लिए सक्रिय",
	"%sortDesc": ": तरह उतरते के लिए सक्रिय",
	"%emptyTbl": "कोई डेटा तालिका में उपलब्ध है",
	"%infoEntr": "दिखा रहा है _START_ _END_ कुल _TOTAL_ प्रविष्टियों की",
	"%infoEmpty": "दिखा रहा है 0 0 कुल 0 प्रविष्टियों की",
	"%infoFilt": "(_MAX_ कुल प्रविष्टियों से फ़िल्टर्ड)",
	"%info1000": ",",
	"%lenMenu": "_MENU_ प्रविष्टियों दिखाएं",
	/* Geomap */
	"%geo-mapcontrol": "नक्शा नियंत्रण",
	"%geo-zoomin": "ज़ूम इन करें",
	"%geo-zoomout": "ज़ूम आउट करें",
	"%geo-zoomworld": "हद तक ज़ूम",
	"%geo-zoomfeature": "तत्व परज़ूमकरें",
	"%geo-scaleline": "नक़्शे का पैमाना",
	"%geo-mouseposition": "माउस कर्सर के अक्षांश और देशांतर",
	"%geo-ariamap": "नक्शा वस्तु. नक्शा सुविधाओं का विवरण नीचे तालिका में हैं.",
	"%geo-accessibilize": "<strong>कुंजीपटल उपयोगकर्ताओं:</strong> जब नक्शा ध्यान में है, तीर कुंजियों का उपयोग करने के लिए नक्शा और प्लस और माइनस चाबियाँ ज़ूम पैन. तीर कुंजियों का नक्शा पैन नहीं जब नक्शा हद तक जूम.",
	"%geo-accessibilizetitle": "निर्देश: मानचित्र नेविगेशन",
	"%geo-togglelayer": "परत के प्रदर्शन टॉगल करें",
	"%geo-hiddenlayer": "इस परत वर्तमान में छिपा हुआ है.",
	"%geo-basemapurl": "http://geoappext.nrcan.gc.ca/arcgis/rest/services/BaseMaps/CBMT_CBCT_GEOM_3978/MapServer/WMTS/tile/1.0.0/BaseMaps_CBMT3978/{Style}/{TileMatrixSet}/{TileMatrix}/{TileRow}/{TileCol}.jpg",
	"%geo-basemaptitle": "BaseMaps_CBMT3978",
	"%geo-basemapurltxt": "http://geoappext.nrcan.gc.ca/arcgis/rest/services/BaseMaps/CBMT_TXT_3978/MapServer/WMTS/tile/1.0.0/BaseMaps_CBMT3978/{Style}/{TileMatrixSet}/{TileMatrix}/{TileRow}/{TileCol}.jpg",
	"%geo-attributionlink": "http://geogratis.gc.ca/geogratis/CBM_CBC?lang=en",
	"%geo-attributiontitle": "GeoGratis - कनाडा आधार नक्शा (अंग्रेजी या फ्रेंच केवल)",
	"%geo-select": "चयन",
	"%geo-labelselect": "नक्शे पर तत्व का चयन करने के लिए जाँच करें",
	/* Disable/enable WET plugins and polyfills */
	"%wb-disable": "मूल HTML संस्करण",
	"%wb-enable": "मानक संस्करण",
	/* Template */
	"%tmpl-signin": "साइन इन करें"
};

window.i18nObj = ind;

})( window );
